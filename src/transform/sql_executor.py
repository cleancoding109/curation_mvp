...
def execute_sql(
    ...
    # NOTE: Do NOT use spark.sparkContext.setJobDescription(description) on serverless/Spark Connect
    # If you need job tracking, use logging or Databricks job tags instead
    try:
        df = spark.sql(sql)
...
def execute_sql_with_temp_view(
    ...
    if view_name in [t.name for t in spark.catalog.listTables()]:
        logger.warning(f"Temp view {view_name} already exists and will be overwritten.")
    df.createOrReplaceTempView(view_name)
    logger.debug(f"Created temp view: {view_name}")
    return execute_sql(spark, sql, description)


def explain_sql(spark, sql: str) -> str:
    """
...
class SqlExecutor:
    ...
    def register_temp_view(self, df: DataFrame, view_name: str):
        ...
        Args:
            ...
            view_name: Name for the temp view
        """
        if view_name in [t.name for t in self.spark.catalog.listTables()]:
            logger.warning(f"Temp view {view_name} already exists and will be overwritten.")
        df.createOrReplaceTempView(view_name)
    ...
    def cleanup_temp_views(self):
        ...
        for view_name in self._temp_views:
            try:
                ...
                logger.error(f"Failed to drop temp view {view_name}: {e}")
